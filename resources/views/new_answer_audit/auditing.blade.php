...
@section('content')
    ...
    <section class="content">
            ...
            <div class="box-body no-padding">
               <div style="width: 200px; float: left">
                ...
                <div style="margin-top: 30px;">
                    <a type="button" href="{{ route('audit_booklist',$data['type']).'?page='.$data['page'] }}" class="btn  btn-primary btn-lg">返回到列表</a>
                    <a class="btn btn-success btn-lg" target="upload" href="{{ route('audit_answer_detail',$data['bookid']).'?from=userAnswer' }}">@if($data['type']==1)修改答案@else上传答案@endif</a>
                    @if($data['type']==0)<a class="btn btn-danger btn-lg" data-bookid="{{ $data['bookid'] }}" id="unable">标记为无法处理</a>@endif
                </div>
            </div>
        ...
        @foreach($data['useranswers'] as $v)
            <div class="box box-info">
                <div class="box-header with-border">
                    <div class="box-header">
                        ...
                        <div class="btn-box"  style="float:right;" data-id="{{ $v->id }}" data-uid="{{ $v->up_uid }}" data-bookid="{{ $v->book_id }}">
                            ...
                            <button type="button" class="btn  btn-primary btn-lg imperfect">答案不完整</button>
                        </div>
                         @elseif($data['type']==1)
                            <div class="btn-box"  style="float:right;" data-id="{{ $v->id }}"  data-bookid="{{ $data['bookid'] }}" data-upuid="{{ $v->up_uid }}">
                                ...
                                <button type="button" class="btn  btn-danger btn-lg  cancel_addblack">撤销并拉黑该用户</button>
                            </div>
                         ...
                         {{--@else
                            <div class="btn-box"  style="float:right;" data-id="{{ $v->id }}" data-uid="{{ $v->up_uid }}" data-bookid="{{ $v->book_id }}">
                            <button type="button" class="btn  btn-success btn-lg update_answers">修改并保存</button>
                            </div>--}}
                        @endif
                    </div>
                ...
                <div class="box-body">
                    ...
                    @if($data['type']!=1)
                        @foreach(explode('|',rtrim($v->answer_img,'|')) as $k=>$img)
                        <div class="img_box" >
                        ...
                        @endforeach
                    @else
                        @foreach($data['answers'] as $k=>$img)
                            <div class="img_box" >
                                <button class="btn btn-xs btn-primary" onclick="rotate(this,270)">左转</button>
                                <button class="btn btn-xs btn-warning" onclick="rotate(this,90)">右转</button>
                                <button class="btn btn-xs btn-success" onclick="rotate(this,180)">倒置</button>
                                <a class="thumbnail">
                                    <img class="answer_pic" data-id="{{ $v->id }}_{{ $k }}"  data-answer-id="{{ $img->id }}" data-src="{{ $img->answer }}" src="{{ config('workbook.thumb_image_url').$img->answer.'?'.time() }}">
                                </a>
                            </div>
                        @endforeach
                    @endif
        ...
        @if($data['type']==0) {{ $data['useranswers']->links() }} @endif
    </section>
@endsection
...
<script>
    ...
    $(function(){
        ...
        $('.not_pass').click(function(){//不通过
            ...
            var status=3;
            axios.post('{{ route('updateStatus') }}',{id,status}).then(response=>{
                if(response.data.status===1){
                    window.location.reload();
                }
            })
        ...
        $('.pass').click(function(){ //通过
            ...
            axios.post('{{ route('answerpass') }}',{id,uid,bookid,all_img}).then(response=>{
                if(response.data.status===1){
                    window.location.href=`{{ route('audit_booklist',[0]) }}`;
                }
            })
        });

        $('.update_answers').click(function(){//修改并保存
            if(!confirm('确认修改并保存?')){
                return false;
            }
            ...
            box_info.find('.answer_pic').each(function () {
                all_img.push($(this).attr('data-src'));
                all_answer_id.push($(this).attr('data-answer-id'));
            });
            axios.post('{{ route('update_answer') }}',{all_img,all_answer_id}).then(response=>{
                if(response.data.status===1){
                     window.location.reload();
                }
            })
        ...
        $('#unable').click(function(){
            ...
            axios.post('{{ route('update_bookstatus') }}',{bookid}).then(response=>{
                if(response.data.status===1){
                window.location.href=`{{ route('audit_booklist',[0]).'?page='.$data['page']  }}`;
             }
            })
        });
        ...
        $('.cancel_pass').click(function(){ //撤销通过
            ...
            axios.post('{{ route('cancel_pass') }}',{id,bookid,up_uid,type}).then(response=>{
                if(response.data.status===1){
                    window.location.href=`{{ route('audit_booklist',[1]).'?page='.$data['page']  }}`;
                }
            });

        });

    $('.cancel_addblack').click(function(){ //撤销通过,并拉黑用户
        var btn_box=$(this).parent('.btn-box');
        var id=btn_box.attr('data-id');
        var bookid=btn_box.attr('data-bookid');
        var up_uid=btn_box.attr('data-upuid');
