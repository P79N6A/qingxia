                                        ...
                                        <td style="width: {{ $now_width }}%" data-id="{{ $book->id }}" class="single_book_box">
                                            <div class="row">
                                                    <div class="col-md-12">
                                                        ...
                                                        @if($book->book_confirm===1) <em class="badge bg-red">book_confirm</em>@else <em class="label label-info">not_book_confirm</em> @endif
                                                        </a>
                                                        ...
                                                        @if($book->sort==0 and $book->book_confirm!=1 and count($book->hasAnswers)>0)
                                                            <a class="btn btn-block btn-danger move_offical_answer" data-grade="{{$data['grade_id']}}" data-subject="{{$data['subject_id']}}" data-volumes="{{$data['volumes_id']}}" data-version="{{$data['version_id']}}" data-id="{{ $book->id }}">设为标准答案</a>
                                                        @endif
...
@push('need_js')
    ...
    <script>
        $(function () {
            ...
            //设置标准答案
            $('.move_offical_answer').click(function () {
                if(!confirm('确认设置为标准答案')){
                    return false;
                }
                let book_id = $(this).attr('data-id');
                let grade_id = $(this).attr('data-grade');
                let subject_id = $(this).attr('data-subject');
                let volumes_id = $(this).attr('data-volumes');
                let version_id = $(this).attr('data-version');
                axios.post('{{ route('ajax_new_repeat_books') }}',{book_id,grade_id,subject_id,volumes_id,version_id})
                    .then(response=>{
                        if(response.data.status===1){
