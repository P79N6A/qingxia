        ...
        <div class="modal-dialog" style="width: 70%;">
            <div class="modal-content">
                <div class="modal-header">
                    <a class="btn btn-primary" v-on:click="page_rotata('left')">向左转</a>
                    <a class="btn btn-primary" v-on:click="page_rotata('right')">向右转</a>
                    <a class="btn btn-primary hide" v-on:click="page_zoom()">放大</a>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button v-on:click="feedback_reply" class="btn btn-primary">确认</button>
    ...
    <section class="content">
        <div class="box box-default color-palette-box">
            <div class="box-header with-border">
                ...
                <a id="add_to_workplace" v-on:click="add_to_workplace()" class="btn btn-primary pull-right" v-if="checkboxModel.length>0"> 添加至工作台(@{{ checkboxModel.length }})</a>
                <a id="del_to_recycle" v-on:click="del_to_recycle()" class="btn btn-danger pull-right" v-if="checkboxModel.length>0">移入回收站(@{{ checkboxModel.length }})</a>
            </div>
            <div class="box-body">
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs">
                        <li class="active"><a href="#tab_1" data-toggle="tab">任务大厅<em class="badge bg-light-blue">@{{ questions_count }}</em></a></li>
                        <li><a href="#tab_2" data-toggle="tab" v-on:click="show_feedback()">反馈站<em class="badge bg-light-blue">@{{ feedback_count }}</em></a></li>
                        <li><a href="#tab_3" data-toggle="tab" v-on:click="show_workplace()">工作台<em class="badge bg-light-blue">@{{ workspace_count }}</em></a></li>
                        <li><a href="#tab_4" data-toggle="tab" v-on:click="show_done()">已完成<em class="badge bg-light-blue">@{{ has_done_count }}</em></a></li>
                        <li><a href="#tab_5" data-toggle="tab" v-on:click="show_recycle()">回收站<em class="badge bg-light-blue">@{{ recycle_count }}</em></a></li>
                    </ul>
                    <div class="tab-content">
                        <div class="tab-pane active" id="tab_1">
                            <table class="table table-bordered">
                                <tr>
                                    <th><label><input type='checkbox' class='input-checkbox' v-model='checkedAll' v-on:click='check_all()'>全选</label></th>
                                    <th>练习册信息</th>
                                    <th>提交图片</th>
                                </tr>
                                <tr v-for="(question ,index) in questions.data">
                                    ...
                                    <td><a class="thumbnail" v-on:click="show_big_pic(question.img)" data-toggle="modal" data-target="#show_big_pic"><img class="responsive question_img" :src="'http://thumb.1010pic.com/'+question.img+'?t='+Date.parse(new Date())"/></a></td>
                                    <td></td>
                                </tr>
                            </table>
                            <ul class="pagination pull-right">
                                <li v-if="questions.current_page==1" class="page-item disabled"><span class="page-link">上一页</span></li>
                                <li v-else class="page-item btn"><a class="page-link" v-on:click="prev_page('questions')" rel="prev">上一页</a></li>
                                <li v-if="questions.current_page<questions.last_page" class="page-item btn" v-on:click="next_page('questions')"><span class="page-link">下一页</span></li>
                        ...
                        <div class="tab-pane" id="tab_2">
                            <table class="table table-bordered" v-if="feedback.length>0">
                                ...
                                <tr v-for="(question ,index) in feedback.data">
                                    ...
                                    <td><a class="thumbnail" v-on:click="show_big_pic(question.img)" data-toggle="modal" data-target="#show_big_pic"><img class="responsive question_img" :src="'http://thumb.1010pic.com/'+question.img"/></a></td>
                                    <td>@{{ question.answer }}</td>
                                    <td>@{{ question.feedback }}</td>
                                    <td>@{{ question.added_at }}</td>
                                    <td>
                                        <a v-if="question.status==0" class="btn btn-xs btn-primary" data-toggle="modal" v-on:click="now_reply(question.id,index)" data-target="#reply_feedback">添加回复</a>
                            ...
                            <ul class="pagination pull-right" v-if="workspace.length>0">
                                ...
                                <li v-else class="page-item btn"><a class="page-link" v-on:click="prev_page('feedback')" rel="prev">上一页</a></li>
                                <li v-if="feedback.current_page<feedback.last_page" class="page-item btn" v-on:click="next_page('feedback')"><span class="page-link">下一页</span></li>
                                <li v-else class="page-item disabled"><a class="page-link" rel="next">下一页</a></li>
                            </ul>

                        </div>
                        <div class="tab-pane" id="tab_3">
                            <table class="table table-bordered" v-if="workspace.length>0">
                                <tr>
                                    <th>提交问题</th>
                                    <th>相关操作</th>
                                </tr>
...
src="{{ asset('images/pageeditor/del.png') }}"/></span></button>
                                ...
                                <td :data-id="question.id" style="text-align: center;width:50%">
                                    ...
                                    <span style="position: relative;">
                                        <strong v-on:click="page_rotate_single(question.id,'left')" class="label label-info">向左转</strong>
                                    <strong v-on:click="page_rotate_single(question.id,'right')" class="label label-info">向右转</strong>
                                    <strong v-on:click="save_pic(question.id,question.img)" class="label label-danger">保存</strong>
                                        <a class="pointer"  v-on:click="show_detail(question.id)">@{{ question.content }}</a>
                                        <strong class="btn btn-xs btn-info" v-on:click="move_to(question.id,'recycle')">移至回收站</strong>
                                        <strong class="btn btn-xs btn-info" v-on:click="move_to(question.id,'workspace')">移至任务大厅</strong>
                                        </span>
                                        <div>
                                ...
                                <td class="answer_tool" :data-qid="question.id">
                                    <div class="nav-tabs-custom">
                                        <ul class="nav nav-tabs">
                                            ...
                                            <li><a data-toggle="tab" :href="'#answer_tool_'+question.id+'_2'" v-on:click="show_answer_area(question.id,'',0)" class="btn btn-xs btn-primary">解答</a></li>
                                        </ul>
                                        <div class="tab-content">
                            ...
                            <ul class="pagination pull-right" v-if="workspace.length>0">
                                ...
                                <li v-else class="page-item btn"><a class="page-link" v-on:click="prev_page('workspace')" rel="prev">上一页</a></li>
                                <li v-if="workspace.current_page<workspace.last_page" class="page-item btn" v-on:click="next_page('workspace')"><span class="page-link">下一页</span></li>
                                <li v-else class="page-item disabled"><a class="page-link" rel="next">下一页</a></li>
                            </ul>
                        </div>
                        <div class="tab-pane" id="tab_4">
                            <table class="table table-bordered" v-if="has_done.length>0">
                                ...
                                <tr v-for="(question ,index) in has_done.data">
                                    ...
                                    <td style="width:50%"><a class="thumbnail" v-on:click="show_big_pic(question.img)" data-toggle="modal" data-target="#show_big_pic"><img class="responsive question_img can_record " :src="'http://thumb.1010pic.com/'+question.img"/></a></td>
                                    {{--<td><div class="jumbotron" v-html="question.answer"></div></td>--}}
                                    <td>
                                        <a class="btn btn-primary btn-xs" v-on:click="show_answer_area(question.qid,question.answer,question.id)">重新编辑</a>
                            ...
                            <ul class="pagination pull-right" v-if="has_done.length>0">
                                ...
                                <li v-else class="page-item btn"><a class="page-link" v-on:click="prev_page('has_done')" rel="prev">上一页</a></li>
                                <li v-if="has_done.current_page<has_done.last_page" class="page-item btn" v-on:click="next_page('has_done')"><span class="page-link">下一页</span></li>
                                <li v-else class="page-item disabled"><a class="page-link" rel="next">下一页</a></li>
                            </ul>
                        ...
                        <div class="tab-pane" id="tab_5">
                            ...
                            <ul class="pagination pull-right" v-if="recycle.length>0">
                                ...
                                <li v-else class="page-item btn"><a class="page-link" v-on:click="prev_page('recycle')" rel="prev">上一页</a></li>
                                <li v-if="recycle.current_page<recycle.last_page" class="page-item btn" v-on:click="next_page('recycle')"><span class="page-link">下一页</span></li>
                                <li v-else class="page-item disabled"><a class="page-link" rel="next">下一页</a></li>
                            </ul>
                ...
                <a class="btn btn-primary" v-on:click="confirm_answer()">保存</a>
                <a class="btn btn-default" v-on:click="cancel_answer()">取消</a>
                    </div>
    ...
    let app = new Vue({
        ...
        updated(){
          ...
          if(this.workspace.length>0){
              if(Editor.$edDiv.length===0){
                  console.log('update');
                  if($('#ed-img').length==0){
//                      $('#ed-img').removeAttr('id');
//                      $('#edit-area').removeAttr('id');
//                      console.log('asdasdas');
                      $('.ed-img:eq(0)').attr('id','ed-img');
                      $('.edit-area:eq(0)').attr('id','edit-area');
                      Editor.init();
                      Editor.initNewImage($('#ed-img').attr('src'));
                  }
                  $(document).on('mousedown','.ed-img',function () {
                      if($(this).attr('id')=='ed-img'){
                          return false;
                      }
                      let source_src = $('#edit-area img').attr('src');
                      let source_id = $('#edit-area').parents('td').attr('data-id');
                      let now_src = $(this).attr('src');
                      let now_id = $(this).parent().attr('data-id');
                      console.log(source_src,source_id,now_src,now_id);
                      let now_box = $(`td[data-id=${now_id}] .edit-area`);
                      $(`td[data-id=${now_id}] .edit-area`).remove();    //移除现有元素
                      //$('#edit-area').attr('style',now_rorate);
                      $('#edit-area').appendTo($(`td[data-id=${now_id}]`));  //将已初始化元素移动至现有元素下
                      $('#edit-area').attr('data-id',now_id);
                      $(`td[data-id=${source_id}]`).append(`<a data-id="${source_id}" class="thumbnail edit-area"><img class="ed-img responsive question_img" data-src="${source_src}" src="${source_src}"></a>`);//填补已初始化元素空缺
                     // http://thumb.1010pic.com/pic18/user_photo/20170902/aab71953c04223d867af67cca0f745ad.jpg?x-oss-process=image/rotate,90
                      //this.$edImg.parent().attr('style').match(/\d+/i);
                      $('#edit-area img').attr('src',now_src);
                      //$('#edit-area a').attr('style',now_rorate);
                      Editor.initNewImage(now_src);
                  });
              }
              //Editor.initNewImage($(this).attr('src'));
              //Editor.initNewImage($(this).attr('src'));
          }
        },
        methods:{
...
//                        $('.ed-img:eq(0)').mousedown();
            ...
            page_rotate_single(id,side){
                ...
                if($(`a[data-id="${id}"] img`).attr('data-src').match(/rotate/)!=null){
                    $(`a[data-id="${id}"] img`).attr('data-src',$(`a[data-id="${id}"] img`).attr('data-src').split('?')[0]);
                }
               $(`a[data-id="${id}"] img`).attr('src',$(`a[data-id="${id}"] img`).attr('data-src')+'?x-oss-process=image/rotate,'+this.now_rotate*90+'&time='+Date.parse(new Date()));
                $('.red-spot').remove();
            ...
            show_done(){
                axios.get('{{ route('api_que_manage_index','has_done') }}').then(response=>{
                    console.log(response.data.has_done);
                    this.has_done = response.data.has_done;
                    this.has_done.length = response.data.has_done.data.length
                }).catch(function (error) {
                    console.log(error);
...
<script>
    ...
    $(document).on('click','.ocr_btn',function () {
        ...
        let qid = $(this).parents('td').attr('data-id');
