                    ...
                    <div class="tab-content">
                        @forelse($data['all_books'] as $key=>$value)
                            ...
                            @forelse($value as $book)
                                    <a target="_blank" class="btn btn-primary btn-xs" href="{{ route('manage_new_oss',['done',$book->id]) }}">{{ $book->bookname }}@if($book->verified_at)<i class="badge bg-blue">已审核</i> @else <i class="badge bg-red">未审核</i>@endif @if($book->answer_not_complete==1) <i class="badge bg-black">答案不全</i> @endif</a>
                            @endforeach
                            </div>
                        @endforeach
                    </div>
    ...
    <script>
        $('#daterange-btn').daterangepicker(
            ...
            function (start, end) {
                ...
                window.location.href = '{{ route('manage_new_oss_status') }}/'+start.format("YYYY-MM-DD")+"/"+end.format("YYYY-MM-DD");
            }
        )
    </script>
@endpush