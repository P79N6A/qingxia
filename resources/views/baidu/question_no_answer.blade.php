        ...
        <div class="box box-primary">
            ...
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    ...
                    <li class="active"><a href="{{ route('baidu_question_no_answer') }}">无答案题目</a></li>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="tab_2">
                        <div class="box-body">
                            ...
                            <hr>
                            <ul class="nav nav-tabs">
                                <li @if($data['now_status']==1)class="active" @endif><a href="{{ route('baidu_question_no_answer',[1,$data['now_type']]) }}">未处理</a></li>
                                <li @if($data['now_status']==2)class="active" @endif><a href="{{ route('baidu_question_no_answer',[2,$data['now_type']]) }}">已处理</a></li>
                            </ul>
				    ...
				    <th>统计数据</th>
                                ...
                                @forelse($data['total'] as $key=>$value)
                                    @php $now_detail = $data['all_answer']->where('md5id',$value->shiti_id)->first();  @endphp
                                    <tr>
                                        <td>{{ $data['now_type'] }}</td>
                                        <td class="shiti_all_info">
                                            <a target="_blank" href="{{ 'http://www.1010jiajiao.com/'.$data['now_type'].'/shiti_id_'.$value->shiti_id }}">{{ 'http://www.1010jiajiao.com/'.$data['now_type'].'/shiti_id_'.$value->shiti_id }}</a>
                                            <div class="question" style="border: 4px solid deepskyblue;border-radius: 4px">
                                                <p>问题：</p>
                                                <div class="now_question">{!! $now_detail->question !!}</div>
                                            </div>
                                            <div class="answer" style="border: 4px solid orangered;border-radius: 4px;">
                                                <p>回答：</p>
                                                <div class="now_answer">{!! $now_detail->answer !!}</div>
                                            </div>
                                            @if(isset($now_detail->analysis))
                                            <div class="analysis" style="border: 4px solid lightseagreen;border-radius: 4px;">
                                                <p>解析：</p>
                                                <div class="now_analysis">{!! $now_detail->analysis !!}</div>
                                            </div>
                                            @endif

                                            <a class="btn btn-primary edit_shiti" data-id="{{ $value->shiti_id }}"  data-type="{{ $data['now_type'] }}">编辑</a>
                                        </td>
    ...
    <script>
        $(function () {
            ...
            $('#get_search').click(function () {
                ...
                let shiti_status = '{{ $data['now_status'] }}';
                window.location.href ='{{ route('baidu_question_no_answer') }}'+'/'+shiti_status+'/'+shiti_type;
            });
...
//            });
            ...
            $(document).on('click','#confirm_shiti',function () {
                ...
                if(shiti_id && shiti_type && question.length>0 && answer.length>0){
                    axios.post('{{ route('baidu_add_shiti_dann') }}',{shiti_id,shiti_type,question,answer,analysis}).then(response=>{
