...
var DT_STATIC = '192.168.0.129';
/*-----------henry-------------*/	
	$(document).on('click',".qdp li",function(){
...
$(function(){
	...
	function addque(ti_que,type)
	{
		...
		timuid=timuid+"_"+Math.round(new Date().getTime()/10);
            ...
            var ti_ans = '';
			var ti_pre='<div class="xiti" id="m_'+timuid+'" uid="'+uid+'">'+order_input+selectText+'<div class="xtq"><div>';
			var ti_next='</div></div></div>';
			var request_pars = "type=addtoWorkbook&timuid="+timuid+"&bookid="+bookid+"&chapterid="+chapterid+"&t_count="+t_count+"&orderid="+orderid+"&qtype="+questionType;
			$.post(request_url+"?"+request_pars , {question:ti_que} , function(data){
				if(data==1)
				{
                    ...
                    $('#timu_box .xiti:last select').change(function() {
                        changeQuestionType(timuid,this.value);
                    });
	...
	function addans(str)
	{
        $(".xiti").each(function(){
            if(!$(this).attr("added"))
            {
                var questionType = this.getElementsByTagName('select')[0].value;
				if($('.ans',this).html())
                    $(this).attr("added",1);
                else {
...
{
    var t = $('tr[data-id="'+timuid+'"]');
    var que=t.children(".for_timu").html();
    que = que.replace(new RegExp(underlineMakeups.replace(/ = /g,' ?= ?').replace(/&nbsp;/g,''),'g'),underlineMakeups);
    var ans=t.children(".for_answer").html();
    var analysis = t.children(".analysis").html();
    var remark = t.children(".remark").html();
    var answerText = ans;
    if(analysis)
        answerText += '<p><br></p>' + analysis;
    if(remark)
        answerText += '<p><br></p>' + remark;
    //$("#E_edit1").html(que);
    //$("#E_edit2").html(ans);
    um1.ready(function(){um1.setContent(que);um2.setContent(answerText);});
    ...
    function t_save()//编辑时保存
    {
        ...
        //"type=t_save&chapterid="+chapterid+"&timuid="+timuid+"&bookid="+bookid;
        ...
        um1.ready(function(){
            ...
            request_pars['answer'] = arr[0];
            // request_pars.push({question:ti_que,answer:arr[0]});
            // arr[0]=arr[0]?arr[0]:'';
            // arr[1]=arr[1]?arr[1]:'';
            // arr[2]=arr[2]?arr[2]:'';
            // data.answer = arr[0],data.analysis = arr[1]||'',data.remark = arr[2]||'';
            axios.post(request_url, request_pars).then(response=>{
                if(response.data.status==1)
                {
                    $('tr[data-id="'+timuid+'"] .for_timu').html(ti_que);
                    $('tr[data-id="'+timuid+'"] .for_answer').html(arr[0]);
...
function auto_timubar()
{
	...
	function auto_search(word)
	{
		...
		$.get(request_url, request_pars, function(content){
			$('#txtbox').html(content);
		});
	}
	
	function gettimulist(gradeid)
	{
...
window.onload = function() {
    ...
    $('#timu_box .xiti select').change(function() {
        changeQuestionType(this.parentNode.parentNode.id.substr(2),this.value);
    });
