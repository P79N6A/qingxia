                ...
                <ul class="nav nav-tabs">
                    ...
                    <li class="active"><a href="#" class="dropdown-toggle" data-toggle="dropdown" >文章<span class="caret"></span></a>
                        <ul class="dropdown-menu">
                            <li class="active"><a href="{{ route('baidu_manage_portal',[$data['time_range']]) }}">热门文章</a></li>
                            <li><a href="{{ route('baidu_new_portal') }}">新建文章</a></li>
                        </ul>
                    </li>
                </ul>
                        ...
                        <span id="get_search" class="input-group-addon btn btn-primary">查询</span>
                            ...
                            <table class="table table-bordered">
                                ...
                                @forelse($data['total'] as $key=>$value)
                                    <tr>
                                        <td class="title">{{ $data['all_descript']->where('id',$value->portal_id)->first()->title }}</td>
                                        <td class="shiti_all_info">
                                            <a target="_blank" href="{{ $value->url }}">{{ $value->url }}</a>
                                            <div class="des">{{ $data['all_descript']->where('id',$value->portal_id)->first()->des }}</div>
                                            <div class="content hide">{!! $data['all_content']->where('id',$value->portal_id)->first()->content !!}</div>
                                            <a class="btn btn-danger edit_portal" data-id="{{ $value->portal_id }}">编辑</a>
                                        </td>
                                        <td class="text-center">
                                            <a href="{{ route('baidu_portal_detail',[$value->portal_id,$data['time_range']]) }}" target="_blank" class="btn btn-danger">查看具体统计信息</a>
    ...
    <script>
        $(function () {
            ...
            $('#get_search').click(function () {
                ...
                window.location.href ='{{ route('baidu_manage_portal') }}'+'/'+time+'/'+shiti_type;
            });
                ...
                if(now_id && title && des.length>0 && contents.length>0){
                    axios.post('{{ route('baidu_add_portal') }}',{now_id,title,des,contents}).then(response=>{
                        if(response.data.status===1){
                            ...
                            let now_tr = now_btn.parent().parent();

                            now_tr.find('.title').html(title);
                            now_tr.find('.des').html(des);
                            now_tr.find('.content').html(contents);
                        }
                        alert(response.data.msg);
                    }).catch(function (e) {
                        console.log(e);
