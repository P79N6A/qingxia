...
class IndexController extends Controller
{
    ...
    //isbn列表
    public function by_isbn($status=0,$start='',$end='',Request $request)
    {
...
//        });
        ...
        if($start==''){
            if($status==0){
                $isbn=$request->isbn;
                if(!$isbn){
                    $data['all_isbn'] = IsbnTemp::where(['uid'=>0,"resultcount"=>0])
                        ->select('isbn','searchnum','sort')
                        ->with('hasWorkBookUserFirst:isbn,cover_img,addtime')->with('hasSearchTemp:isbn,searchnum')
                        ->with('hasIsbnDetail:isbn,id,print_description,preg_sort_id,preg_grade_id,preg_subject_id,preg_volumes_id,preg_version_id,real_bookname')
                        ->with('hasIsbnDetail.has_sort:id,name')
                        ->withCount('hasOfficalBook')
                        ->withCount('hasWorkBookUser')
                        ->orderBy('searchnum','desc')->paginate(10);
                }else{
                    $data['all_isbn'] = IsbnTemp::where(['uid'=>0,"resultcount"=>0,'isbn'=>$isbn])
                        ->select('isbn','searchnum','sort')
                        ->with('hasWorkBookUserFirst:isbn,cover_img,addtime')
                        ->with('hasSearchTemp:isbn,searchnum')
                        ->with('hasIsbnDetail:isbn,id,print_description,preg_sort_id,preg_grade_id,preg_subject_id,preg_volumes_id,preg_version_id,real_bookname')
                        ->with('hasIsbnDetail.has_sort:id,name')
                        ->withCount('hasOfficalBook')
                        ->withCount('hasWorkBookUser')
            ...
            elseif($status==1){
                $data['all_isbn'] = IsbnTemp::where(function($query){
                    $query->where('uid','>',0);
                    $query->where("resultcount",0);
                })
                    ->select('isbn','searchnum','uid','bookname','grade_id','subject_id','version_year','volumes_id','version_id','sort','cover_photo')
                    ->with('hasSearchTemp:isbn,searchnum')
                    ->with('hasIsbnDetail:isbn,id,print_description')
                    ->with('has_sort:id,name')
            ...
            if($status==0){
                /*$data['all_isbn'] = IsbnTemp::from("a_tongji_search_isbn_temp2")
                    ->join("a_tongji_search_isbn_temp2_everyday as d","a_tongji_search_isbn_temp2.isbn","d.isbn")
                    ->where([ 'a_tongji_search_isbn_temp2.uid'=>0])
                    ->where("d.addtime",">=",$start)
                    ->where("d.addtime","<",$end)
                    ->select('a_tongji_search_isbn_temp2.isbn','d.searchnum','a_tongji_search_isbn_temp2.uid')
                    ->with('hasWorkBookUserFirst:isbn,cover_img')
                    ->with('hasIsbnDetail:isbn,id,print_description,preg_sort_id,preg_grade_id,preg_subject_id,preg_volumes_id,preg_version_id,real_bookname')
                    ->with('hasIsbnDetail.has_sort:id,name')
                    ->with('hasOfficalBook')
                    ->withCount('hasWorkBookUser')
                    ->with('hasSearchEveryday:isbn,searchnum')
                    ->orderBy('searchnum','desc')
                    ->paginate(10);
                dd($data['all_isbn']);*/
                $data['all_isbn'] = IsbnTempEveryday::from("a_tongji_search_isbn_temp2_everyday")
                    ->join("a_tongji_search_isbn_temp2","a_tongji_search_isbn_temp2_everyday.isbn","a_tongji_search_isbn_temp2.isbn")
                    ->where(function($query) use($start,$end){
                        if($start==$end){
                            $query->where('a_tongji_search_isbn_temp2_everyday.addtime','=',$start);
                        }else{
                            $query->where("a_tongji_search_isbn_temp2_everyday.addtime",">=",$start);
                            $query->where("a_tongji_search_isbn_temp2_everyday.addtime","<",$end);
                        }
                    })
                    ->where([ 'a_tongji_search_isbn_temp2.uid'=>0,'a_tongji_search_isbn_temp2.resultcount'=>0])
                    ->select('a_tongji_search_isbn_temp2_everyday.isbn',DB::raw('sum(a_tongji_search_isbn_temp2_everyday.searchnum) as searchnum'))
                    ->with(['hasWorkBookUserFirst'=>function($query){
                        return $query->select('isbn','cover_img','addtime')->orderBy('addtime','desc')->where('addtime','>=','2018-01-01');
                    }])
                    ->with('hasIsbnDetail:isbn,id,press,print_description,preg_sort_id,preg_grade_id,preg_subject_id,preg_volumes_id,preg_version_id,real_bookname')
                    ->with('hasIsbnTemp:isbn,uid,id')
                    ->groupBy('a_tongji_search_isbn_temp2_everyday.isbn')
                    ->orderBy('searchnum','desc')
                    ->paginate(10);
                foreach($data['all_isbn'] as $k=>$v){
                    $data['all_isbn'][$k]['has_offical_book_count'] = AWorkbook1010::where(['isbn'=>$v['isbn'],'status'=>1])->orwhere(['isbn'=>$v['isbn'],'status'=>7])->count();
                    $data['all_isbn'][$k]['has_work_book_user_count'] = PreMWorkbookUser::where([['isbn',$v['isbn']],['addtime','>=','2018-01-01']])->count();
                    if($v->hasIsbnDetail?$v->hasIsbnDetail->press:false){
                        $re=DB::connection('mysql_local')->table('book_version')->select('name')->where('name','like',$v->hasIsbnDetail->press)->first();
                        $data['all_isbn'][$k]['press']=$re?$re->name:'';
                    }else{
                        $data['all_isbn'][$k]['press']='';
                    }
                    $desc=$v->hasIsbnDetail?$v->hasIsbnDetail->print_description:'';
                    if(strstr($desc,'暑假')){
                        $data['all_isbn'][$k]->hasIsbnDetail->preg_volumes_id=6;
                    }
                ...
                $data['all_isbn'] = IsbnTempEveryday::from("a_tongji_search_isbn_temp2_everyday")
                    ->leftJoin("a_tongji_search_isbn_temp2","a_tongji_search_isbn_temp2_everyday.isbn","a_tongji_search_isbn_temp2.isbn")
                    ->where(function($query)use($start,$end){
                        if($start==$end){
                            $query->where('a_tongji_search_isbn_temp2.op_time','=',strtotime($start));
                        }else{
                            $query->where("a_tongji_search_isbn_temp2.op_time",">=",strtotime($start));
                            $query->where("a_tongji_search_isbn_temp2.op_time","<",strtotime($end.'24:00:00'));
                        }

                    })
                    ->where( 'a_tongji_search_isbn_temp2.uid','>',0)
                    ->where('a_tongji_search_isbn_temp2.resultcount','0')
                    ->select(DB::raw('any_value(a_tongji_search_isbn_temp2.op_time) as op_time'),'a_tongji_search_isbn_temp2_everyday.isbn',DB::raw('sum(a_tongji_search_isbn_temp2_everyday.searchnum) as searchnum'))
                    ->with('hasCover1010:isbn,cover,id,newname,grade_id,subject_id,version_year,volumes_id,version_id,sort')
                    ->with('hasIsbnDetail:isbn,preg_sort_id,print_description')
                    ->with('hasCover1010.has_sort:id,name')
                    ->with('hasIsbnTemp:isbn,uid,id')/*bookname,grade_id,subject_id,version_year,volumes_id,version_id,sort*/
                    ->with('hasIsbnTemp.has_user:id,name')
                    ->groupBy('a_tongji_search_isbn_temp2_everyday.isbn')
                    ->orderBy('searchnum','desc')
                    ->paginate(10);
                //dd($data['all_isbn']);
                foreach($data['all_isbn'] as $k=>$v){
                    $data['all_isbn'][$k]['has_offical_book_count'] = AWorkbook1010::where(['isbn'=>$v['isbn'],'status'=>1])->orwhere(['isbn'=>$v['isbn'],'status'=>7])->orwhere(['isbn'=>$v['isbn'],'status'=>14])->count();
                    $data['all_isbn'][$k]['has_work_book_user_count'] = PreMWorkbookUser::where('isbn',$v['isbn'])->count();
                }
                //dd($data['all_isbn']);
            }
        }
    ...
    //dd($data['all_isbn']);
        ...
        return view('new_answer_audit.index',compact('data'));
    }
    ...
    public function isbn_detail($isbn)
    {
        ...
        $data['isbn_detail'] = PreMWorkbookUser::where([['isbn',$isbn],['status','in','0,1,9']])->select('id','sort_name','grade_id','subject_id','grade_id','volumes_id','version_id','version_year','sort_id as sort','cover_img','cip_img','hdid')->orderBy('grade_id','asc')->orderBy('subject_id','asc')->get();
        return view('new_answer_audit.isbn_detail',compact('data'));
    }
    ...
    public function by_answer()
    {
        ...
        return view('new_answer_audit.by_answer',compact('data'));
    }

    //答案详情
    /*public function answer_detail($to_book_id)
    {
        ...
        $data['related_books'] = AWorkbook1010::where($related)->select('id','bookname','version_year','grade_id','subject_id','volumes_id','version_id','sort','collect_count','concern_num','cover','cip_photo','isbn')->with(['has_answer'=>function($query){
    ...
    public function answer_detail($isbn)
    {
        $data['isbn'] = $isbn;
        $data['list']=PreMWorkbookUser::where(["isbn"=>$isbn,"status"=>0])->select('id','up_uid')->with("answers:book_id,answer_img")->get();
        foreach ($data['list'] as $key=>$value){
            $data['user_message'][$key] = PreMHomeworkMessage::where(['type'=>'answer_audit','hid'=>$value->id,'huid'=>$value->up_uid])->select(['uid','huid','add_time','msg'])->get();
        }
         $data['bookid'] = AWorkbook1010::where('isbn',$isbn)->orderBy('id','desc')->select('id')->first();

        if($data['bookid']){
            $data['need_buy']=AWorkbook1010::where('id',$data['bookid']->id)->select('newname')->with("has_newonly:newname,need_buy")->first();
        }
        //dd($data);
        return view('new_answer_audit.answer_detail',compact('data'));
    }
...
//            }
            ...
            return response()->json(['status'=>1,'msg'=>'操作成功','zip'=>'/uploads/'.\Auth::id().'/'.$isbn.'/'.$zipFileName]);
