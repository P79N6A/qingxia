var Draw = {
    ...
    init: function ($img) {
        //$($img).offsetParent().attr('style','transform: rotate(-90deg)');
        //$($img).attr('style','transform: rotate(-90deg)');
        this.now_width = $img[0].width;
        this.now_height = $img[0].height;
    ...
    getConstraints: function () {
        if(this.$edImg.parent().attr('style')){
            let rotate = this.$edImg.parent().attr('style').match(/\d+/i);
            console.log(rotate);
            if(parseInt(rotate[0])/90%2===1){
                Editor.now_width = this.$edImg[0].height
                Editor.now_height = this.$edImg[0].width ;
            }else{
                Editor.now_height = this.$edImg[0].height;
                Editor.now_width = this.$edImg[0].width;
            }
        }
        this.edImg_b.t = this.$edImg.offset().top;
        this.edImg_b.b = this.$edImg.offset().top + Editor.now_height;
        this.edImg_b.l = this.$edImg.offset().left;
        this.edImg_b.r = this.$edImg.offset().left + Editor.now_width;

        this.parentOffset.x = this.$edImg.parent().offset().left;
        this.parentOffset.y = this.$edImg.parent().offset().top;
        console.log(11111111,this.parentOffset.x,this.parentOffset.y,111111111);

    },

    mousedown: function (e) {//新建区域按下鼠标触发
    ...
    start: function (e) {
        ...
        if (e.pageX > this.edImg_b.l) {
            ...
            if (e.pageX > this.edImg_b.r) {
                w = Editor.now_width - parseFloat(this.curSpot.$edSpot.css('left'));
                console.log(parseFloat(this.curSpot.$edSpot.css('left')));
            }
        ...
        if (e.pageY > this.edImg_b.t) {
            ...
            if (e.pageY > this.edImg_b.b) {
                h = Editor.now_height - parseFloat(this.curSpot.$edSpot.css('top'));
            }
    ...
    mouseup: function (e) {//鼠标松开创建区域
        this.isDraw = false;
        if (this.curSpot) {//console.log(this.curSpot);
            Draw.indexsort = $('.timu_sort').length+1;
      ...
      //  $(`.spot-options:eq(${Editor.pic_index})`).show();
    }
...
Spot.prototype.unselect = function () {
    ...
    if (this.spotClass === 'img-spot') {
        var $spotImg = this.$edSpot.find('> img').eq(0),
            ...
            spotImgTop = this.$edSpot.position().top,
            spotImgH = $spotImg.height(),
            edImgW = Editor.now_width,
            edImgH = Editor.now_height;
...
var Editor = {
    ...
    init: function () {
        ...
        this.$edImg = $(`#ed-img`);
        this.now_width = $(`#ed-img`)[0].width;
        this.now_height = $(`#ed-img`)[0].height;
...
//console.log(this.spotPool);
        ...
        if ($('#img-responsive').is(':checked')) {
            var imgW = this.now_width,
                imgH = this.now_height;
    ...
    cloneSpot: function () {
        ...
        var imgDim = {w: this.now_width, h: this.now_height},
     ...
     */
    newSpot: function (spot) {//console.log(spot);
        this.spotPool.push(spot);
		var pageval=Draw.timu_page>0?Draw.timu_page:Draw.page;
        //var timu_sort_val = $('.timu_sort').length+1;
spot.$edSpot.html('<input class="timu_sort hide" value='+Draw.indexsort+' type="text"><input class="hide timu_page" value='+pageval+' type="text">');
        //select when mouse down
    ...
    updateSpotDim: function () {
        ...
        console.log('updateSpotDim',w,h,'updateSpotDim');
    ...
    updateSpotPos: function () {
        ...
        console.log('updateSpotPos',x,y,'updateSpotPos');
