    ...
    <section class="content">
        <div class="box box-default color-palette-box">
                        ...
                        <div style="height:500px;width: 500px;overflow-y: auto">
                            <table class="table table-bordered">
                                ...
                                <tr>
                                    ...
                                    <th>出版社别名</th>
                                    <th>出版社地区</th>
                                </tr>@foreach($data['all_version'] as $value)
                                    <tr>
                                        <td>{{ $value->name }}</td>
                                        <td>{{ $value->press_name }}</td>
                                        <td>{{ $value->press_alias }}</td>
            ...
            <div class="box-body">
                ...
                <div class="dropdown">
                    <button class="btn btn-lg btn-danger dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
                        选择系列
                        <span class="caret"></span>
                    </button>
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
                        @forelse($data['now_sort'] as $key=>$sort)
                            @if($sort->not_num!=0)
                                <li><a href="{{ route('book_new_index',[$data['type'],$sort->sort]) }}">{{ $sort->has_sort?$sort->has_sort->name:'未分类' }}<em class="badge bg-light-blue">{{ '原有数量：'.$sort->num }}</em><em class="badge bg-red">
                                            @if($key%3===0)
                                                黄少敏
                                            @elseif($key%3===1)
                                                苏蕾
                                        ...
                                        <em class="badge bg-light-blue">现有数量：{{ $sort->not_num }}</em>
                                    </a></li>
                            @endif
                            @endforeach
                    </ul>
                ...
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs">
                        <li @if($data['type']==='unfinished') class="active" @endif><a
                                    href="{{ route('book_new_index','unfinished') }}">未整理</a></li>
                        <li @if($data['type']==='finished') class="active" @endif><a
                                    href="{{ route('book_new_index','finished') }}">已整理</a></li>
                    </ul>
                                ...
                                @foreach($data['all_book'] as $book)
                                    <div class="box-body well single_book_info" data-id="{{ $book->id }}">
                                        ...
                                        <div class="col-md-6 book_info_box" data-id="{{ $book->id }}">
                                            ...
                                            @if(!$book->has_sort && $book->sort>0)
                                                <div class="input-group">
                                                <input type="text" data-sort="{{ $book->sort }}" value="" class="form-control" placeholder="输入系列名"/>
...
//                $(`.book_info_box[data-id="${book_id}"]`).find(`select[data-name="${type}"]`).append(option).trigger("change");
            ...
            //保存系列名
            $('.save_sort_name').click(function () {
                let book_id = $(this).parents('.book_info_box').attr('data-id');
                let sort_id = $(this).prev().attr('data-sort');
                let sort_name = $(this).prev().val();
                console.log(sort_id);
                axios.post('{{ route('book_new_workbook_api','add_sort_name') }}',{sort_id,sort_name}).then(response=>{
                    if(response.data.status===1){
                        $(`.single_book_info[data-id=${book_id}]`).remove();
                    }
                });
