    ...
    getConstraints: function () {
        if(this.$edImg.parent().attr('style')){
            ...
            if(parseInt(rotate[0])/90%2===1){
                Editor.rotate = parseInt(rotate[0])/90;
            }else{
                Editor.rotate = 0;
            }
        }
    ...
    start: function (e) {
        ...
        if (!this.curSpot) {
            if(Editor.rotate===0){
                ...
                console.log('has_rotate');
                this.curSpot = new Spot(this.type, this.mouseCoord.x - this.parentOffset.x,this.mouseCoord.y - this.parentOffset.y);
            }
        }
        console.log(this.mouseCoord.x - this.parentOffset.x);
        console.log(this.mouseCoord.y - this.parentOffset.y);
...
var Spot = function (spotClass, x, y) {
    ...
    $('#edit-area').addClass('border-huge');

    this.$edSpot = $('<div>').addClass(spotClass)
        .css('position', 'absolute')
        .css('left', x+'px')
        .css('top', y+'px')
        .css('transform',`rotate(${Editor.rotate*90}deg)`)
        //.data('spotObj', this)
        //.appendTo(Editor.$edDiv);
        .appendTo($('#edit-area'));
    console.log(this.$edSpot);
