            ...
            <div class="box-body">
                ...
                @if($data['start']=='')
                    <div class="input-group input-group-sm" style="width:200px;">
                        <input type="text" class="form-control isbn_search">
                    <span class="input-group-btn">
                      <button type="button" class="btn btn-info btn-flat" id="isbn_search">搜isbn</button>
                    </span>
                    </div>
                @endif
                    ...
                    @foreach($data['all_isbn'] as $isbn)
                        @php if($isbn->hasIsbnDetail){
                            ...
                            if($isbn->hasWorkBookUserFirst){
                                $bookuser_img=starts_with($isbn->hasWorkBookUserFirst->cover_img,'http://')?$isbn->hasWorkBookUserFirst->cover_img:config('workbook.user_image_url').$isbn->hasWorkBookUserFirst->cover_img;
                        ...
                        dd($isbn);
                        @endphp
                        <div class="col-md-6 info_box" data-isbn="{{ $isbn->isbn }}" data-all_id="{{ $isbn->hasIsbnDetail? $isbn->hasIsbnDetail->id:-1 }}" @if($data['start']!='') data-temp2_id="{{ $isbn->hasIsbnTemp->id }} @endif">
                            <div class="panel panel-primary">
                                ...
                                <div class="panel panel-body">
                                    ...
                                    <p class="col-md-12">
                                        <div class="col-md-6">
                                        <a class="thumbnail">
                                            <img class="answer_pic" src="@if($data['status']==1 && $data['start']==''){{ $isbn->cover_photo }}@elseif($data['status']==1 && $data['start']!=''){{ $isbn->hasCover1010[0]->cover}}@else{{ $bookuser_img }}@endif" alt="" />
                                        </a>
                                        @if($data['status']==0)
                                            <div>求助时间：{{ $isbn->hasWorkBookUserFirst?$isbn->hasWorkBookUserFirst->addtime:'' }}</div>
                                        @else
                                            <div>处理时间：{{ $isbn->op_time?date('Y-m-d H:i:s',$isbn->op_time):'' }}</div>
                                        @endif
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="input-group" style="width: 100%">
                                                ...
                                                @if($data['status']==1 && $data['start']==''){{$isbn->bookname}}@elseif($data['status']==1 && $data['start']!=''){{ $isbn->hasCover1010[0]->newname }}@else{{ $real_bookname }}@endif" value="@if($data['status']==1 && $data['start']==''){{$isbn->bookname}}@elseif($data['status']==1 && $data['start']!=''){{  $isbn->hasCover1010[0]->newname }}@else{{ $real_bookname }}@endif">
                                            </div>
                                            <div class="input-group pull-left">
                                                <label class="input-group-addon">年份</label>
                                                <input type="text" maxlength="4" class="form-control version_year" value="@if($data['status']==1 && $data['start']==''){{$isbn->version_year}}@elseif($data['status']==1 && $data['start']!=''){{ $isbn->hasCover1010[0]->version_year }}@else{{2018}}@endif">
                                            </div>
                                                ...
                                                <select data-name="grade" class="grade_id form-control select2 pull-left " >
                                                    @forelse(config('workbook.grade') as $key => $grade)
                                                        ...
                                                        @elseif($data['status']==1 && $data['start']!='')
                                                            @if($isbn->hasCover1010[0]->grade_id==$key)
                                                                selected="selected"
                                                            @endif>{{ $grade }}</option>
                                                        @else
                                            ...
                                            <div class="input-group pull-left">
                                                ...
                                                <select data-name="subject" class="subject_id form-control select2">
                                                    ...
                                                    @forelse(config('workbook.subject_1010') as $key => $subject)
                                                        ...
                                                        @elseif($data['status']==1 && $data['start']!='')
                                                            @if($isbn->hasCover1010[0]->subject_id==$key)
                                                                selected="selected"
                                                            @endif>{{ $subject }}</option>
                                                        @else
                                            ...
                                            <div class="input-group pull-left">
                                                ...
                                                <select data-name="volumes" class="volumes_id form-control select2">
                                                    @forelse(config('workbook.volumes') as $key => $volume)
                                                        ...
                                                        @elseif($data['status']==1 && $data['start']!='')
                                                            <option value="{{ $key }}"
                                                            @if($isbn->hasCover1010[0]->volumes_id==$key ) selected="selected"
                                                                    @endif>{{ $volume }}</option>
                                                        @else
                                                ...
                                                <div class="input-group pull-left">
                                                    ...
                                                    <select data-name="version" class="version_id form-control select2">
                                                        ...
                                                        @forelse(cache('all_version_now') as $key => $version)
                                                            ...
                                                            @elseif($data['status']==1 && $data['start']!='')
                                                                <option value="{{ $version->id }}"
                                                                @if($isbn->hasCover1010[0]->version_id==$version->id ) selected="selected"
                                                                        @endif>{{ $version->name }}</option>
                                                            @else
                                                                @if($version->id>-1)
                                                                    <option value="{{ $version->id }}"
                                            ...
                                            <div class="input-group pull-left">
                                                ...
                                                <select class="select2 form-control ssort_select" >
                                                    ...
                                                    @forelse(\App\OneModel\ASubSort::where([['sort_id',$isbn->sort],['ssort_name','!=','']])->select(['ssort_id','ssort_name'])->get() as $ssort)
                                                        <option value="{{ $ssort->ssort_id }}">{{ $ssort->ssort_name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="input-group">
                                                ...
                                                <select data-name="sort" class="form-control sort_name select2">
                                                    ...
                                                    @elseif($data['status']==1 && $data['start']!='')
                                                        <option value="{{ $isbn->hasCover1010[0]->sort }}" selected>{{ $isbn->hasCover1010[0]->has_sort->name }}</option>
                                                    @else
                                            ...
                                            <div class="btn btn-group">
                                               @if($data['status']==1 && $data['start']!='')
                                                    <a data-id="{{ $isbn->hasCover1010[0]->id }}" class="change_book btn btn-danger">修改</a>
                                               @elseif($data['status']==1 && $data['start']=='')
                                            ...
                                            <div>
                                                @if($data['status']===1)
                                                @php $has_user = $isbn->has_user; @endphp
                                                <p>处理人：@if($data['start']==''){{ $has_user?$has_user->name:'' }}@else{{ $isbn->hasIsbnTemp->has_user?$isbn->hasIsbnTemp->has_user->name:'' }}@endif</p>
                                                @endif
    ...
    <script>
        $(function () {
           /* var lazyLoadInstances = [];
            ...
            $('#isbn_search').click(function(){
               var isbn=$('.isbn_search').val();
                    window.location.href=`{{ route('new_audit_index',[$data['status']]) }}?isbn=${isbn}`;
                });
            ...
            $('.change_book').click(function(){
                ...
                var book_name = box.find('.book_name').val();
                if(book_name.includes('未选择') || book_name.includes('待定')){
                    return false;
                }
                var grade_id = box.find('.grade_id').val();
                var subject_id = box.find('.subject_id').val();
                var volumes_id = box.find('.volumes_id').val();
                var version_id = box.find('.version_id').val();
                var sort_id = box.find('.sort_name').val();
                var version_year=box.find('.version_year').val();
                var cover=box.find('.answer_pic').attr('src');
...
//            let volumes_info = {1:'上册',2:'下册',3:'全一册',4:'中考用书',5:'高考用书',6:'暑假作业',7:'寒假作业',8:'小升初用书',9:'必修1',10:'必修2',11:'必修3',12:'必修4',13:'必修5',15:'选修1',16:'选修2',17:'选修3',18:'选修4'}
        ...
        $(document).on('change','.sort_name',function () {
            ...
            $(document).on('click','.save_book',function () {
                var box = $(this).parents('.info_box');
                alert(box.find('.ssort_select').val());
                alert(box.find('.ssort_select option:selected').text());
                return false;
                ...
                let ssort_name = box.find('.ssort_select').text();
                var isbn = box.attr('data-isbn');
                var all_id=box.attr('data-all_id');
                var temp2_id=box.attr('data-temp2_id');
                var book_name = box.find('.book_name').val();
                if(book_name.includes('未选择') || book_name.includes('待定')){
                    return false;
                }
                ...
                var version_year=box.find('.version_year').val();
                var cover=box.find('.answer_pic').attr('src');
                axios.post('{{ route('ajax_new_audit_list','save_bookinfo') }}',{temp2_id,all_id,isbn,book_name,grade_id,subject_id,volumes_id,version_id,sort_id,version_year,cover,ssort_id,ssort_name})
                    .then(response=>{
                        if(response.data.status===1){
                            box.remove();
                        }else{
